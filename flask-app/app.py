...
def add_notification(notification_type, message, target_user=None, target_post=None):
    ...
    # Store reference IDs based on notification type
    if target_user:
        notification['target_user_id'] = target_user['_id']
    if target_post:
        notification['target_post_id'] = target_post['_id']
        notification['link'] = url_for('view_topic', post_id=target_post['_id'])  # Add a link to the post
    notifications.insert_one(notification)
...
def dashboard_assign_moderator():
    ...
    # Add a notification for the user being assigned as a moderator
    add_notification(
        'moderator_assigned',
        f"🚀 {session['username']} assigned a moderator.",
        target_user=user
    )
